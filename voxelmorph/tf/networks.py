    ...
    def __init__(
        ...
        **kwargs,
        ...
        if seg_downsize is not None:
            warnings.warn(
                "seg_downsize is deprecated, use the seg_resolution parameter."
            )
            seg_resolution = seg_downsize

        # configure base voxelmorph network
        vxm_model = VxmDense(
            inshape, nb_unet_features=nb_unet_features, bidir=bidir, **kwargs
        )

        # configure downsampled seg input layer
        inshape_ds = (np.array(inshape) / seg_resolution).astype(int)
        seg_src = tf.keras.Input(
            shape=(*inshape_ds, nb_labels), name=f"{name}_source_seg"
        )

        # configure warped seg output layer
        seg_flow = layers.RescaleTransform(
            1 / seg_resolution, name=f"{name}_seg_resize"
        )(vxm_model.references.pos_flow)
        y_seg_src = layers.SpatialTransformer(
            interp_method="linear", indexing="ij", name=f"{name}_seg_transformer"
        )([seg_src, seg_flow])
        ...
        if bidir_labels:
            ...
            # target seg input
            seg_trg = tf.keras.Input(
                shape=(*inshape_ds, nb_labels), name=f"{name}_target_seg"
            )
            inputs.append(seg_trg)

            # warp labels bidirectionally
            neg_seg_flow = layers.RescaleTransform(
                1 / seg_resolution, name=f"{name}_seg_neg_resize"
            )(vxm_model.references.neg_flow)
            y_seg_trg = layers.SpatialTransformer(
                interp_method="linear",
                indexing="ij",
                name=f"{name}_seg_neg_transformer",
            )(
                [seg_trg, neg_seg_flow]
            )  # nopep8
...
class VxmDenseSemiSupervisedPointCloud(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        single_pt_trf = lambda x: utils.point_spatial_transformer(
            x, sdt_vol_resize=sdt_vol_resize
        )

        # vxm model
        vxm_model = VxmDense(
            inshape, nb_unet_features=nb_unet_features, bidir=True, **kwargs
        )
        ...
        subj_dt_input = tf.keras.Input(
            [*sdt_shape, nb_labels_sample], name="subj_dt_input"
        )
        ...
        if surf_bidir:
            ...
            subj_surf_input = tf.keras.Input(
                surface_points_shape, name="subj_surface_input"
            )
            warped_subj_surf_pts = KL.Lambda(single_pt_trf, name="warped_subj_surface")(
                [subj_surf_input, neg_flow]
            )

            atl_dt_input = tf.keras.Input(
                [*sdt_shape, nb_labels_sample], name="atl_dt_input"
            )
...
class InstanceDense(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        flow_layer = ne.layers.LocalParamWithInput(
            shape=(*ds_warp_shape, len(inshape)), mult=mult
        )
        ...
        if int_steps > 0:
            pos_flow = layers.VecInt(method="ss", name="flow_int", int_steps=int_steps)(
                pos_flow
            )

            # resize to final resolution
            if int_resolution > 1:
                pos_flow = layers.RescaleTransform(int_resolution, name="diffflow")(
                    pos_flow
                )

        # warp image with flow field
        y_source = layers.SpatialTransformer(
            interp_method="linear", indexing="ij", name="transformer"
        )([source, pos_flow])

        # initialize the keras model
        super().__init__(
            name="vxm_instance_dense", inputs=[source], outputs=[y_source, preint_flow]
        )
...
class ProbAtlasSegmentation(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # build warp network
        vxm_model = VxmDense(
            inshape, nb_unet_features=nb_unet_features, src_feats=nb_labels, **kwargs
        )
        ...
        stat_logssq = MaxPooling(pool_size=max_pool, name="logssq_pooling")(
            stat_logssq_vol
        )
        ...
        if init_mu is not None:
            ...
            stat_mu = KL.Lambda(
                lambda x: network_stat_weight * x + init_mu, name="comb_mu"
            )(stat_mu)
        ...
        if init_sigma is not None:
            ...
            stat_logssq = KL.Lambda(
                lambda x: network_stat_weight * x + init_logsigmasq, name="comb_sigma"
            )(stat_logssq)
        ...
        uloglhood = KL.Lambda(lambda x: unnorm_loglike(*x), name="unsup_likelihood")(
        ...
        logpdf = KL.Lambda(lambda x: log_pdf(*x), name="log_pdf")(
            [uloglhood, warped_atlas]
        )
        ...
        def logsum(logpdf):
            ...
            return alpha + tf.math.log(
                tf.reduce_sum(ked, -1, keepdims=True) + K.epsilon()
            )
        ...
        super().__init__(
            inputs=[vxm_model.inputs[1], vxm_model.inputs[0]] + vxm_model.inputs[2:],
...
class TemplateCreation(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        warp_input_model = tf.keras.Model(
            inputs=[source_input], outputs=[atlas_tensor, source_input]
        )
        ...
        # initialize the keras model
        super().__init__(
            inputs=[source_input], outputs=[y_source, y_target, mean_stream, pos_flow]
        )
    ...
    def apply_transform(self, src, trg, img, interp_method="linear", fill_value=None):
        ...
        y_img = layers.SpatialTransformer(
            interp_method=interp_method, fill_value=fill_value
    ...
    def __init__(
        ...
        **kwargs,
        ...
        atlas_gen = Conv(
            ...
            bias_initializer=KI.RandomNormal(mean=0.0, stddev=1e-7),
        )(last)
        ...
        if templcondsi:
            ...
            atlas_tensor = KL.Lambda(
                lambda x: K.concatenate([x[0], x[1][..., 1:]]), name="atlas"
            )([x_img, atlas_tensor])
        else:
        ...
        warp_input_model = tf.keras.Model(
            inputs=inputs, outputs=[atlas_tensor, source_input]
        )
        ...
        if use_mean_stream:
            ...
            mean_stream = ne.layers.MeanStream(name="mean_stream", cap=mean_cap)(
                neg_flow
            )
...
class Transform(tf.keras.Model):
    ...
    def __init__(
        ...
        if affine:
            ...
            trf_shape = (
                inshape if rescale is None else [int(d / rescale) for d in inshape]
            )
        ...
        trf_scaled = (
            trf_input
            if rescale is None
            else layers.RescaleTransform(rescale)(trf_input)
        )

        # transform and initialize the keras model
        trf_layer = layers.SpatialTransformer(
            interp_method=interp_method, name="transformer", fill_value=fill_value
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for n in range(nb_hyp_layers):
            hyp_last = KL.Dense(
                nb_hyp_units, activation="relu", name="%s_hyp_dense_%d" % (name, n + 1)
            )(hyp_last)

        # attach hypernetwork to vxm dense network
        hyp_model = tf.keras.Model(
            inputs=hyp_input, outputs=hyp_last, name="%s_hypernet" % name
        )
